...
$exampleFile = $dataDir . '/products.json.example';
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $jsonStr = json_encode($products, JSON_PRETTY_PRINT | JSON_UNESCAPED_UNICODE);
    $written = @file_put_contents($dataFile, $jsonStr, LOCK_EX);
    if ($written === false) {
        http_response_code(500);
        echo json_encode(["success" => false, "error" => "Failed to write products to disk. Check folder write permissions for data/ directory."]);
        exit;
    }
